...
(function($) {
	...
	$.fn.lightBox = function(settings) {
		...
		settings = jQuery.extend({
			...
			imageLoading:			'/resource/img/lightbox/loading.gif',		// (string) Path and the name of the loading icon
			imageBtnPrev:			'/resource/img/lightbox/prev.gif',			// (string) Path and the name of the prev button image
			imageBtnNext:			'/resource/img/lightbox/next.gif',			// (string) Path and the name of the next button image
			imageBtnClose:			'/resource/img/lightbox/close.gif',		// (string) Path and the name of the close btn
			imageBlank:				'/resource/img/lightbox/blank.gif',			// (string) Path and the name of a blank image (one pixel)
			...
			imageArray:				[],
			activeImage:			0,
            showDetails:            true, //ReadyScript addition
            titleAttr:              'title' //ReadyScript addition
		},settings);
		...
		function _start(objClicked,jQueryMatchedObj) {
			...
			if ( jQueryMatchedObj.length == 1 ) {
				settings.imageArray.push(new Array(objClicked.getAttribute('href'),objClicked.getAttribute(settings.titleAttr)));
			} else {
				// Add an Array (as many as we have), with href and title atributes, inside the Array that storage the images references		
				for ( var i = 0; i < jQueryMatchedObj.length; i++ ) {
					settings.imageArray.push(new Array(jQueryMatchedObj[i].getAttribute('href'),jQueryMatchedObj[i].getAttribute(settings.titleAttr)));
				}
			}
...
if ($.contentReady) {
    $.contentReady(function() {
        $("[rel='lightbox-tour']", this).lightBox();
    });
}