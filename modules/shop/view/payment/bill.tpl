...
<div style="font-weight: bold; font-size: 16pt; padding-left:5px;">
    Счет № {$pay->getOption('number_prefix')}{$order.order_num} от {$order.dateof|date_format:"%d.%m.%Y"}</div>
    ...
    <tr>
        ...
        <td>
            ...
            <div style="font-weight:bold;  padding-left:2px;">
                {if $user.is_company}
                    {$user.company}, ИНН: {$user.company_inn}, Тел.: {$user.phone}
                {else}
                    {$user.surname} {$user.name} {$user.midname}, Тел: {$user.phone}
                {/if}
            </div>
        </td>
    </tr>
...
<br />
<div>
Всего наименований {count($order_data.items)} на сумму {$order_data.total_cost} рублей.<br />
