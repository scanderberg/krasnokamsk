...
{  
    ...
    var methods = {
        init: function(initoptions) {
            ...
            $this
                .on('click', '.status-bar a', changeStatus)               
                .on('change', 'select[name="use_currency"]', methods.refresh)
                .on('click', 'a.tool.remove', remove)
                .on('click', 'a.tool.addcoupon', addCoupon)
                .on('click', 'a.tool.addproduct', addProduct)
            ...
            $('.pr-table a[rel="lightbox-products"]').lightBox({
    ...
    */
    showChangeOffer = function(){
        ...
        $('.product-offer', item_div).show();
        $('.multiOffers', item_div).show();
    },
    /**
    ...
    */
    ...
    changeStatus = function() {
        $('.status-bar .act', $this).removeClass('act');
        $(this).closest('.top').addClass('act');
        
        $('#status_text', $this).html( $(this).html() );
        $('#status', $this).val( $(this).data('id') );
    },
    ...
    remove = function() {
        $('.pr-table tr:has(input[name="chk[]"]:checked)', $this).remove();
        methods.refresh();
    },
    ...
    addCoupon = function() {
        ...
        $('.added-items').append($input.attr('name', 'items['+uniq+'][title]').val('Купон на скидку '+code).clone());
        methods.refresh();
    },
    ...
    addProduct = function() {
        ...
        $('.product-group-container').selectProduct({
            ...
            // По закрытию диалога
            onResult: function(){
                var cost_id = $('select[name=costtype]').val();
