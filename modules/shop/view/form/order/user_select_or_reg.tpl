<input name="is_reg_user" type="radio" value="0" id="link-user" {if !$elem.is_reg_user}checked{/if}><label for="link-user">Связать с зарегистрированным пользователем</label><br>
<input name="is_reg_user" type="radio" value="1" id="reg-user" {if $elem.is_reg_user}checked{/if}><label for="reg-user">Зарегистрировать нового пользователя</label><br>
<br>
