   ...
   $(function() {
        ...
        function init(){ 
            ...
            myPlacemark = new ymaps.Placemark([{$warehouse.coor_x}, {$warehouse.coor_y}], {
                hintContent: '<div><p><b>{$warehouse.title}</b></p>{if !empty($warehouse.adress)}<p>адрес: {$warehouse.adress}</p>{/if}{if !empty($warehouse.phone)}<p>тел.:{$warehouse.phone}</p>{/if}{if !empty($warehouse.work_time)}<p>время работы: {$warehouse.work_time}</p>{/if}</div>',
                balloonContent: '<div><p><b>{$warehouse.title}</b></p>{if !empty($warehouse.adress)}<p>адрес: {$warehouse.adress}</p>{/if}{if !empty($warehouse.phone)}<p>тел.:{$warehouse.phone}</p>{/if}{if !empty($warehouse.work_time)}<p>время работы: {$warehouse.work_time}</p>{/if}</div>'
            });
