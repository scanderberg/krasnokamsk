            ...
            var checkboxChange = function(e, trigged)
            {
                ...
                if (this.checked) {
                    ...
                    if (!trigged) { //Если это событие не вызвали мы сами же при открытии диалога
                        inputContainer.append('<input type="hidden" name="'+fieldName+'[group][]" value="'+val+'" catids="'+parents_str+'">');
                    ...
                    selectedContainer.append(li_product);
                        ...
                        $("input[catids*=',"+val+",']",inputContainer).each(function()
                        {
        ...
        var make = function()
        {
            ...
            var loadProducts = function(optVars)
            {
                ...
                $.ajaxQuery({
                    ...
                    data: $.extend(optVars, { hideProductCheckbox: hideProductCheckbox }),
                    success: function(response) {
            ...
            var onStartClick = function(e)
            {
                ...
                if (options.selectButtonText !== false) {
                    buttons[options.selectButtonText] = onPressOk;
                }
            ...
            var checkboxChange = function()
            {
                ...
                if (this.checked) {
                    inputContainer.append('<input type="hidden" name="'+fieldName+'[product][]" data-weight="'+$(this).data('weight')+'" catids="'+$(this).attr('catids')+'" value="'+val+'">');
            ...
            var bindEvents = function()
            {
                ...
                $('.product-list tbody tr', dialog).mouseover(function() {
                    $(this).addClass('over');
                });
                
                $('.product-list tbody tr', dialog).mouseout(function() {
            ...
            var updateProductCheckbox = function() 
            {
                ...
                if (checked_dirs.length>0) {
                    $(options.tableProducts+' input', dialog).each(function() {
                        ...
                        var product_dirs = $(this).attr('catids');
                        var checked = false;
