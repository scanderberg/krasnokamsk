{addcss file="{$mod_css}photoblock.css" basepath="root"}
...
{else}
    <div class="photo_block" method="POST" enctype="multipart/form-data" action="{adminUrl mod_controller="photo-blockphotos" pdo="addphoto" linkid=$param.linkid type=$param.type}">
        ...
        <div class="upload-block">
            ...
            <button class="delete-list" type="button" formaction="{adminUrl mod_controller="photo-blockphotos" pdo="delphoto"}">удалить</button>
            <div class="dragzone">{t}Чтобы начать загрузку, перетащите сюда фотографии{/t}</div>
        </div>
        
        <ul class="photo-list overable" data-sort-url="{adminUrl mod_controller="photo-blockphotos" pdo="movephoto" do=false}" data-edit-url="{adminUrl mod_controller="photo-blockphotos" do=false pdo="editphoto"}">
            {$photo_list_html}
