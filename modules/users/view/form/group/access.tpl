...
<div class="switch-site-access">
    <label for="site_admin">{t}Включить доступ к администрированию текущего сайта{/t}</label> <input id="site_admin" type="checkbox" name="site_access" value="1" {if !empty($site_access)}checked{/if}>
</div>

<div class="hrline">
    <div class="fullaccleft"><input type="checkbox" name="menu_access[]" value="{$smarty.const.FULL_USER_ACCESS}" {if isset($menu_access[$smarty.const.FULL_USER_ACCESS])}checked{/if} id="full_user"> <label for="full_user">{t}Полный доступ к меню{/t}</label></div>
    <div class="fullaccright"><label for="full_admin">{t}Полный доступ к меню{/t}</label><input type="checkbox" name="menu_admin_access[]" value="{$smarty.const.FULL_ADMIN_ACCESS}" {if isset($menu_access[$smarty.const.FULL_ADMIN_ACCESS])}checked{/if} id="full_admin"> </div>
    <strong>{t}Доступ к пунктам меню{/t}</strong>
</div>

<div class="treeblock">
    <div class="lefttree localform" style="position:relative">
        <div class="overlay" id="user_overlay">&nbsp;</div>
        <div class="label">{t}Меню пользователя{/t}</div>
        <div class="wrap">            
            {$user_tree->getView()}
        </div>
    </div>               
    <div class="righttree localform" style="position:relative">
        ...
        <div class="label">{t}Меню администратора{/t}</div>
        <div class="wrap">
            {$admin_tree->getView()}
        </div>
...
<div class="hrline">
    <div class="fullaccleft"><input type="checkbox" name="module_access[{$smarty.const.FULL_MODULE_ACCESS}]" value="255" {if isset($module_access[$smarty.const.FULL_MODULE_ACCESS])}checked{/if} id="full_module"> <label for="full_module">{t}Полный доступ ко всем модулям{/t}</label></div>
    <strong>{t}Права к модулям{/t}</strong>
</div>
...
function onTabFocus()
{
    //для IE 6 - устанавливаем высоту оверлеев
    userfull.defaultDraw();
    adminfull.defaultDraw();
    modulefull.defaultDraw();
}
</script>
{/literal}
{/if}