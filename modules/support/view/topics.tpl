...
<table class="supportTopics">
    ...
    <tbody>
        ...
        <tr data-id="{$item.id}">
            ...
            <td class="msgCount"><span class="text">сообщений:</span> {$item.msgcount}{if $item.newcount>0} (новых: {$item.newcount}){/if}</td>
            <td class="toolsCol">
                <a href="{$router->getUrl('support-front-support', ["Act" => "delTopic", "id" => $item.id])}" class="remove" title="Удалить переписку по этой теме"></a>
            </td>
        </tr>
        {/foreach}
    </tbody>
