...
<h1 class="fn">{$product.title}</h1>
        ...
        <div class="fcost" align="center" style="background-color: #2072bf!important;">
			...
			/ {$product.offers.items[0]->getUnit()->stitle} - поставить количество можно будет в корзине.
...
{if $product->inDir('kirpich')}<br><span style="color: red!important; font-size: 14px!important;">Кирпич продается поддонами!</span>{/if}
